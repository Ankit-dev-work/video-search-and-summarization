...
def advance(update: AliasUpdate, runner: Runner = command_runner) -> None:
    ...
    if update.digest and digest != update.digest:
        # The source was verified moments ago, and create copies the manifest
        # pointer exactly -- a differing created digest means the content tag
        # moved between verification and create. Fail closed and loudly.
        raise RuntimeError(
            f"{update.name}: created alias digest {digest!r} != release-set "
            f"{update.digest!r} -- source moved between verification and create"
        )
