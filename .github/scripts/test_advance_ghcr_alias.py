...
class AdvanceTest(unittest.TestCase):
    ...
    @staticmethod
    def _runner(commands, source_digest=DIGEST, created=DIGEST, write_metadata=True):
        ...
        def runner(command: list[str]) -> str:
            ...
            if "create" in command:
                if write_metadata:
                    ...
                    path.write_text(
                        json.dumps({"containerimage.descriptor": {"digest": created}})
                    )
                return ""
    ...
    def test_created_digest_mismatch_fails_closed(self):
        """create copies the manifest pointer exactly, so a created digest
        that differs from the verified release-set digest means the source
        moved between verification and create."""
        commands: list[list[str]] = []
        with self.assertRaisesRegex(RuntimeError, "source moved between"):
            advance(self._update(), self._runner(commands, created=MIRROR_DIGEST))
