...
- **Video-embedding producer** — required only when the embedding-downsampling processor runs (search path): it consumes chunked video embeddings from the embed topic. Owned by the video-embedding microservice (`vss-deploy-video-embedding/references/integrate-vss-deploy-video-embedding.md`).
- **ELK (Elasticsearch / Logstash / Kibana)** — required for the outputs to be queryable/visualized. **Logstash consumes Behavior Analytics' output topics** (`mdx-incidents`, `mdx-frames`, `mdx-behavior`, `mdx-embed-filtered`) via protobuf codecs and indexes them into ES (`mdx-frames-*`, `mdx-behavior-*`, `mdx-incidents-*`, etc.). Not a hard `depends_on` of Behavior Analytics itself, but required for the pipeline to be observable. Owned by ELK (`vss-build-vision-agent/references/services/elk.md`).
- **Alert Microservice (`alert-bridge`)** — downstream consumer of `mdx-incidents` / `mdx-alerts` when Behavior Analytics is the candidate-alert source for VLM verification. The `category` field on emitted incidents is the join key into `alert_type_config.json`. Owned by the Alert Microservice (`vss-manage-alerts/references/integrate-alerts.md`).
...
> **Orchestrator-side composition (decoupling).** Behavior Analytics is owned by the `vss-setup-behavior-analytics` skill, so how it is *selected and wired into a composed profile* is not carried here. That lives in the orchestrator's own service reference, `vss-build-vision-agent/references/services/behavior-analytics.md` (with the per-profile compose keys in `references/profiles/`). This file is the neutral integration contract only.
