...
6. Before writing delta artifacts or starting a stock or delta deployment, present a compact architecture diagram in the conversation. Show the Foundation, added and removed capability owners and service keys, principal data flows and topics, external endpoints, and GPU/model placement. When the selected flow publishes to Kafka, state whether each requested topic already exists in `kafka-topic-init-container`'s default `KAFKA_TOPICS` or needs a custom definition; specifically, state that `mdx-vlm-captions` is already present and do not propose a custom definition for it. Do not save the diagram as a build artifact.
